...
# Two-step (OAuth client behind the proxy): exchange for a 1h token via the
# proxy, then mint against the public API (the proxy injects Authorization on
# every request, so only the exchange goes through it).
