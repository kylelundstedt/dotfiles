...
_tok=$(curl -fsS -u "$(op read 'op://Employee/Tailscale OAuth/Client ID' --account industryvault.1password.com):$(op read 'op://Employee/Tailscale OAuth/Client secret' --account industryvault.1password.com)" -d grant_type=client_credentials https://api.tailscale.com/api/v2/oauth/token | jq -r .access_token)
