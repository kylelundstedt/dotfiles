...
- [ ] **Migrate LLM gateway onto an AC appliance VM** — IN EXECUTION 2026-07-21: Phases 1–3 done + gated (repo `kylelundstedt/llm-gateway`, VM live at `https://llm-gateway.dojo-sun.ts.net`, both providers verified, token in 1P); Phase 4 partial. **Blocked on operator:** double `sudo fdesetup authrestart` acceptance test + healthchecks.io `llm-gateway` project/ping URL. Then: wire URL + re-enable health timer, power-loss drill, Phase 5 cutover + 7-day burn-in, Phase 6 teardown. Full state in [agent_docs/llm-gateway-migration.md](agent_docs/llm-gateway-migration.md) "Execution log"
