...
- [ ] AgentsView fleet pilot — **Phase 0 + two canaries live 2026-07-22**: mini collector (`v0.38.1`) has 122 sessions across mini/`iv-docs`/`iv-sandbox`; authenticated tailnet sync, exact sampled fidelity, online snapshot, and isolated restore verified. Remaining before fleet rollout: re-auth mini GitHub CLI and push newest dotfiles/`iv-image` commits, move tokens into 1Password/Keychain, add external Healthchecks ping, prove a newly-created session arrives <10m, measure one day of sync traffic, then inventory/roll out the fleet. See [agent_docs/agentsview-pilot.md](agent_docs/agentsview-pilot.md)
