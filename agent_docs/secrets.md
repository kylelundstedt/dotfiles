...
| --------------------- | ---------- | ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `api-tailscale`       | http-proxy | `tag:tailnet` (private dev VMs only) | OAuth client creds (Basic) → 1h token → join keys. Standing on private dev VMs via the `tailnet` tag; time-boxed `--for 30m` per VM for anything public and for prod-lane targets; `join-tailnet` attaches-then-detaches for one-offs. Was `(none)` 07-28..08-19, then on 19 VMs incl. 4 public until 2026-09-15 — see exe-dev-remediation Finding 2 |
...
**Tailscale OAuth client** (U11, 2026-07 — replaced the expiring API key AND the static `iv-internal-*` auth keys; nothing expires anymore): scopes `Auth Keys: Write` + `Devices Core: Write` as configured in 2026-07 — **but measured 2026-09-15 the client gets HTTP 403 on `GET /tailnet/-/devices` and `/acl`, so treat it as auth-keys-only until re-checked in the console** — tags `tag:dev` **and `tag:prod`** (exeslim's boot-time `iv-tailnet-join` mints `tag:prod` through it; an earlier version of this line said `tag:dev` only). Tailscale checks tag _ownership_, not just scope: `tagOwners` must list the client as an owner of each tag it mints, or the mint fails with `requested tags [...] are invalid or not permitted`. The raw client secret is NOT accepted as a static Bearer/Basic API credential — consumers do the standard OAuth exchange (`POST /api/v2/oauth/token`, `client_secret_basic`) for a 1h token. The exe.dev integration injects `Authorization: Basic base64(client_id:client_secret)`, so VM flows exchange THROUGH the proxy and then hit the public API with the token. If the client is ever compromised/rotated:
