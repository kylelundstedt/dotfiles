...
- MCP servers use remote HTTP transport. OAuth servers (motherduck, tigris) work everywhere. GitHub servers (github-home, github-work) use PATs from 1Password (macOS only). `hub-mcp` (personal-mcp) is the local exception: `install.sh` registers it on personal Macs only — `http://127.0.0.1:8765/mcp` on klundstedt-mini, the tailnet URL on other Macs that can reach it.
...
| `launchd/`        | LaunchAgents for macOS — scheduled jobs: repo sync, personal-mcp ingest + server, Tigris backup (stowed) |
